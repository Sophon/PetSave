...
import com.example.petsaveedu.common.data.api.model.ApiAnimal
...
class ApiAnimalMapper @Inject constructor(
    ...
    private fun parseSize(size: String?): Size {
        ...
        return Size.valueOf(size.uppercase(Locale.ROOT))
    }
    ...
    private fun mapMedia(apiAnimal: ApiAnimal): Media {
        return Media(
            photos = apiAnimal.photos.orEmpty().map { apiPhotoMapper.mapToDomain(it) },
            videos = apiAnimal.videos.orEmpty().map { apiVideoMapper.mapToDomain(it) }
        )
    }
}